...
df = pd.read_excel("rule_based_classification/miuul_gezinomi.xlsx")
df.head()
df.info()
df.shape
df["SaleId"].nunique()
...
df.groupby(["SaleCityName","ConceptName" ])["Price"].mean().unstack()
...
df["EB_Score"] = pd.cut(df["SaleCheckInDayDiff"],
...
df.groupby(["SaleCityName", "ConceptName","EB_Score"]).agg({"Price": ["mean", "count"]}).unstack()
df.groupby(["SaleCityName", "ConceptName","Seasons"]).agg({"Price": ["mean", "count"]}).unstack()
df.groupby(["SaleCityName", "ConceptName","CInDay"]).agg({"Price": ["mean", "count"]}).unstack()
...
agg_df["sales_level_based"] = [(row[0] + "_" + row[1] + "_" + row[2]).upper() for row in agg_df.values]
#agg_df["sales_level_based"] = agg_df[["SaleCityName", "ConceptName", "Seasons"]].agg(lambda x: "_".join(x).upper(), axis=1)
agg_df.shape
agg_df["sales_level_based"].nunique()
...
new_customer = "GIRNE_YARIM PANSIYON_LOW"
agg_df[agg_df["sales_level_based"] == new_customer]